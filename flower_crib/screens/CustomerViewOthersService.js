...
const timeSlotsO = ['10:00 AM', '11:00 AM', '12:00 PM', '1:00 PM', '2:00 PM', '3:00 PM', '4:00 PM', '5:00 PM'];

export default function ViewOtherService({ category, time, price, id }) {
  ...
  const [availableSlots, setAvailableSlots] = useState(timeSlotsO);

  useEffect(() => {
    const bookingRef = collection(db, 'Booking');
    const q = query(bookingRef, where('date', '==', date));
    onSnapshot(q, (snapshot) => {
      const slots = timeSlotsO.filter((slot) => {
